...
const Store = ({ isEditMode = true }) => {
            ...
            sides: {
              left: {
                ...
                shelves: Array.from({ length: 4 }, (_, shelfIndex) => ({
                  ...
                  slots: Array.from({ length: 3 }, (_, slotIndex) => {
                    const slotId = `A${aisles.length}-R${index + 1}-L-SH${
                      shelfIndex + 1
                    }-S${slotIndex + 1}`;
                    return {
                      id: slotId,
              ...
              right: {
                ...
                shelves: Array.from({ length: 4 }, (_, shelfIndex) => ({
                  ...
                  slots: Array.from({ length: 3 }, (_, slotIndex) => {
                    const slotId = `A${aisles.length}-R${index + 1}-R-SH${
                      shelfIndex + 1
                    }-S${slotIndex + 1}`;
                    return {
                      id: slotId,
                ...
                racks: aisle.racks.map((rack) => {
                  if (rack.id === rackData.id) {
                    ...
                    if (rackData.updatedSlot) {
                      const { side, shelfIndex, slotIndex, ...slotData } =
                        rackData.updatedSlot;
                      ...
                      updatedRack.sides = {
                        ...
                        [targetSide]: {
                          ...
                          shelves: rack.sides[targetSide].shelves.map(
                            (shelf, sIndex) => {
  ...
  const focusCameraOnRack = (aisleId, rackId, side) => {
    ...
    const rackRotation = rack.rackDegree
      ? (rack.rackDegree * Math.PI) / 180
      : 0;
    ...
    if (cameraRef.current && controlsRef.current) {
      ...
      const animateCamera = () => {
        ...
        cameraRef.current.position.z =
          startPosition[2] +
          (targetPosition[2] - startPosition[2]) * easeProgress;
    ...
    >
      ...
      {isEditMode && (
        <Sidebar
          ...
          handleAddAisle={() =>
            setIsPlacingRegularAisle(!isPlacingRegularAisle)
          }
      ...
      {searchResult.visible && (
        <div
          style={{
            ...
            left: "30px",
            padding: "12px 20px",
            backgroundColor: searchResult.isSuccess
              ? "rgba(46, 125, 50, 0.9)"
              : "rgba(198, 40, 40, 0.9)",
            color: "white",
            borderRadius: "8px",
      ...
      <Canvas
        ...
        style={{
          ...
          width: "100%",
          height: "100%",
          cursor: isEditMode && isPlacingRegularAisle ? "crosshair" : "default",
        }}
        gl={{
          antialias: true,
      ...
      >
        ...
        <OrbitControls
          ...
          enableDamping
          dampingFactor={0.05}
          enabled={!(isEditMode && isPlacingRegularAisle)}
        ...
        />
        <pointLight position={[-10, 10, -10]} intensity={0.3} distance={50} />
        ...
        <mesh
          ...
          position={[0, -0.5, 0]}
          receiveShadow
          onClick={isEditMode ? handlePlaneClick : null}
        >
        ...
        {aisles?.map((aisle, index) => (
          <group
            key={aisle.id}
            onClick={isEditMode ? (e) => handleAisleClick(e, aisle.id) : null}
          >
            <Aisle
              ...
              rotation={[
                0,
                aisle.aisleDegree ? (aisle.aisleDegree * Math.PI) / 180 : 0,
                0,
              ]}
              highlightedProduct={
                highlightedProduct && highlightedProduct.aisleId === aisle.id
                  ? highlightedProduct
                  : null
              }
            />
